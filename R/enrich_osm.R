  ...
  if (measure_name == "spherical") {
    if (length(ftr_geometry) == 0) {
     ...
     matrix <- rep(Inf, nrow(ref_geometry))
      ...
      if (exact_distance){
        # Calculate indeces of neighbors within a radius r + 25% 
        #(+25% since the distances are not exact). 
        #We re-calculate distances next and filter during the kernel transformation.
        ids <- dbscan::frNN(x = m_ftr_geometry, query = m_ref_geometry, sort = FALSE,
                            eps = r*1.25)$id
        # Calculate neighbors
        ids_coord <- lapply(ids, function(i) {ftr_geometry[i]} )

        #Calculate correct distances (slow)
        ...
        # Calculate euclidean distances (fast)
